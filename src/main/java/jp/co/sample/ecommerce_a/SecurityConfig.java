...
public class SecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		http.authorizeRequests() // 認可に関する設定 TODO:permitAll()を下にする。
				.antMatchers("/order/toOrder", "/order/toOrderComplete","/downloadCsv").authenticated() // ログインが必要なのは、注文確認画面のみ
		...
		http.formLogin() // ログインに関する設定
				...
				.failureUrl("/?error=true") // ログイン失敗に遷移させるパス //TODO:エラーページ作成?
				.defaultSuccessUrl("/showItem/index", true)
		...
		http.logout() // ログアウトに関する設定
				...
				.logoutSuccessUrl("/showItem/index") // ログアウト後に遷移させるパス(ここではログイン画面を設定)
